...
The Location API microservice is responsible for exposing an API for clients to get on-demand location updates.
...
```
bazel build //projects/location-service/location-api/cmd/location-api
```
...
```
bazel build //projects/location-service/location-api/cmd/location-api --platforms=@io_bazel_rules_go//go/toolchain:linux_amd64 
```
...
```
bazel run //projects/location-service/location-api/cmd/location-api:location-api.binary
```
